...
def new_post():
    if request.method == "GET":
        try:
            user_id = login_session["userid"]
            return render_template("new_post.html", user=user_id, catalogs=CATALOGS)
        except:
...
def catalog_posts(catalog):
    ...
    return render_template("catalog_menu.html", user=user_id,
        catalog=int(catalog), posts=posts, catalogs=CATALOGS)
...
def post_detail(post_id):
    ...
    return render_template("post_detail.html", user=user_id, post=post, catalogs=CATALOGS)
...
def edit_post(post_id):
    try:
        user_id = login_session["userid"]
        try:
            post = session.query(Post).filter_by(id=post_id, author=user_id).one()

            if request.method == "GET":
                return render_template("post_edit.html", post=post, catalogs=CATALOGS)
            else:
